...
fn fixture_missing_behavior(msg: &str) -> bool {
    // Mirror `ntfs` crate behavior, but allow an AFF-specific env var too.
    if env::var_os("AFF_TESTDATA_REQUIRED").is_some()
        || env::var_os("NTFS_TESTDATA_REQUIRED").is_some()
    {
        panic!("{msg}");
    }
...
///
/// If `AFF_TESTDATA_REQUIRED=1` (or `NTFS_TESTDATA_REQUIRED=1`) is set, missing/placeholder
/// fixtures will **panic** (useful for CI). Otherwise, the caller should `return` early from the
/// test to skip it.
